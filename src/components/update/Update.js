...
export default function Update() {
  ...
  const [confPassword, setConfPassword] = useState({password: "", showPassword: false,});
  ...
  const update = ()=>{
    if(password.password === confPassword.password){
      axios.put("http://localhost:3010/User/update/"+token._id+"",
    {name: name, lastName: lastName, email: email, password: password.password? password.password: token.password, dateOfBirth: dateOfBirth, gender: gender})
    .then(
      res =>{
    ...
    <Grid>
        <Paper elevation={20} sx={{padding:"30px 20px", width:300, margin:"20px auto"}}>
          ...
          <TextField
                ...
                endAdornment={
                  <InputAdornment position="end">
                    <IconButton
                      ...
                      edge="end"
                    >
                      {confPassword.showPassword ? <VisibilityOff /> : <Visibility />}
                    </IconButton>
                  </InputAdornment>
                }
            ...
            <Button variant="contained" onClick={update} fullWidth color="primary">Update</Button>
            <Button variant="contained" onClick={handleNavHome} fullWidth color="primary">Cancel</Button>
            </Stack>
